...
{
    int numb = 0;	// Порядковый номер ячейки вектора Zero Array, которая содержит "самый большой" ноль
...
{
    // Копируем текущую матрицу array, перед тем как провести выключение ребра
    for (int i = 0; i < sizeArr; i++)
    {
        for (int j = 0; j < sizeArr; j++)
        {
    ...
    //Исключаю ребро и сдвигаю элементы
    int I = 0, J = 0;
    for (int i = 1; i < sizeArr; i++)
    {
        I = i;
    ...
    // Обновляем (уменьшаем на 1) размер матрицы array
    --sizeArr;

    // Провожу смещение
    // Первые строка и столбец матрицы содержат в себе названия соответствующих строк и столбцов
...
void SalesmansMethod::mainFunction()
{
    ...
    while (flag)
    {
        ...
        // Выбираем ноль с наибольшим весом
